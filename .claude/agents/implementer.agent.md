...
tools: Read, Edit, Write, Grep, Glob, Bash, Agent, TodoWrite, mcp__context7
skills:
...
Para duvidas sobre a API do Godot 4.x (nodes, sinais, metodos built-in), use as ferramentas MCP `context7` para buscar a documentacao oficial atualizada em vez de confiar em memoria.
...
1. **Antes de implementar** — Consulte a spec relevante no vault via `obsidian` CLI (ex: `obsidian search query="..."`, `obsidian read file="..."`) para entender os requisitos.
2. **Durante a implementação** — Se descobrir ambiguidades ou decisões de design necessárias, registre no vault (`obsidian append` / `obsidian create`) ou sinalize ao usuário.
