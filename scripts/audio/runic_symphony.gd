...
func play_rune_sound(rune: RuneInstance, activation_count: int) -> void:
	...
	if rune.data.custom_sound:
		var id = _playback.play_stream(rune.data.custom_sound, 0.0, element_bank.base_volume_db, pitch)
		if id >= 0:
	...
	var volume_db: float = element_bank.base_volume_db
	if note_count > 1:
